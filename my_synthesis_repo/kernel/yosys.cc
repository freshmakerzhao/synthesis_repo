...
#ifdef HYBRDLINK
	bool flag_enable_compatibility_mode=false;
	bool flag_fasm_encryption=false;
	bool flag_is_pb_file=false;
#endif
int autoidx = 1;
int yosys_xtrace = 0;
...
#ifdef HYBRDLINK
	int counter = 0;  // To skip the first line of ABC log if counter == 0
...
void rewrite_filename_to_pb(std::string &filename){
	...
	filename = filename + "pb";  // name the serialized file to .pb suffix
}
...
#ifdef HYBRDLINK
		Tcl_SetResult(interp, (char *)"Synthesizer command produced an error", TCL_STATIC);
#else
...
#ifdef HYBRDLINK
		log_error("uncaught exception during Synthesizer command invoked from TCL\n");
#else
...
#ifdef HYBRDLINK
	log_error("Can't determine synthesizer executable path\n.");
#else
...
#  else
	...
	proc_share_path = proc_self_path + "../share/" + proc_program_prefix()+ YOSYS_EXE_NAME + "/";
	if (check_file_exists(proc_share_path, true)) {
		yosys_share_dirname = proc_share_path;
		return;
	}
...
void run_pass(std::string command, RTLIL::Design *design)
{
	if (design == nullptr)
		design = yosys_design;
#ifndef HYBRDLINK
	log("\n-- Running command `%s' --\n", command.c_str()); // log for debug only. Commented for release
#endif // HYBRDLINK
	Pass::call(design, command);
}
