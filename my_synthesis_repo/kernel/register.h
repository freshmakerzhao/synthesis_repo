...
#define SERIALIZE_STATIC_FILES 0
...
struct Frontend : Pass
{
	...
	bool deserialize_pb_file(std::stringstream &ss, std::string filename);
	static void decrypt_pb_content_stringstream(std::ifstream *ff, std::istringstream *&iss);
	static void decrypt_pb_content_stringstream(std::ifstream &ff, std::istringstream *&iss);
...
struct Backend : Pass
{
	...
	static bool serialize_file_to_pb(std::stringstream &ss, std::vector<std::string> path_seg, std::string &original_filename);
	static bool serialize_single_file_to_pb(std::string &original_filename ,std::string &pb_filename);

	static std::string yosys_exe_path;
};
