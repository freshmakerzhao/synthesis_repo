...
std::string getCurrentWorkingDir() {
    ...
    if (getcwd(temp, sizeof(temp)) != nullptr) {
		...
		Yosys::log_error("Unable to get cwd!\n");
        return "";
    }
}
...
void Frontend::extra_args(std::istream *&f, std::string &filename, std::vector<std::string> args, size_t argidx, bool bin_input)
{
	...
	if (argidx < args.size())
	{
		...
		if (filename.compare(0, 2, "<<") == 0) {
			...
			std::vector<std::string> words = split_str(filename,'.');
			std::string file_surfix;
			if (words.size())
				file_surfix = words[words.size() - 1];


			// Decrypt serialized file
			std::istringstream *iss = nullptr;
			bool is_decrypt = bin_input && file_surfix == "pb";
			if (is_decrypt)
				decrypt_pb_content_stringstream(ff, iss);

#if SERIALIZE_STATIC_FILES
			if (ff != nullptr){

				// Generate new file name with new surfix: .pb
				std::vector<std::string> temp_words = split_str(filename,'.');
				size_t surfix_length = temp_words[temp_words.size() -1].length();
				filename.erase(filename.length() - surfix_length, surfix_length);
				std::string new_filename = filename + "pb";  // name the serialized file to .pb suffix

				// Recover original file name in filename variable
				filename = filename + temp_words[temp_words.size() -1];  // change back to the original file name

				// Transfer ifstream to istream for serialization
				std::stringstream ss;
				ss << ff->rdbuf();

				// Split full path by '/', pass to serialization function
				temp_words = split_str(new_filename,'/');
				Backend::serialize_file_to_pb(ss, temp_words, filename);
			}
			// (not in use) 临时对指定文件进行加密，如 brams_defs.vh
			//
			// std::ifstream *ff2 = new std::ifstream;
			// std::string filename2 = "/home/zhaoshuai/Source/NEXT/yosys/techlibs/xilinx/brams_defs.vh";
			// ff2->open(filename2, std::ifstream::in);
			// if (ff2->fail()) {
			// 	delete ff2;
			// 	ff2 = nullptr;
			// }
			...
			// ss2 << ff2->rdbuf();
			// std::string new_filename2 = "/home/zhaoshuai/Source/NEXT/yosys/techlibs/xilinx/brams_defs.pb"; // name the serialized file to .pb suffix
			// std::vector<std::string> temp_words2 = split_str(new_filename2,'/');
			// Backend::serialize_file_to_pb(ss2, temp_words2, filename2);

			// (in use) 临时对指定文件进行加密，如 brams_defs.vh
			// std::string temp_origin_filename = "/home/zhaoshuai/Source/NEXT/yosys/techlibs/xilinx/brams_defs.vh";
			// std::string temp_pb_filename = "/home/zhaoshuai/Source/NEXT/yosys/techlibs/xilinx/brams_defs_2.pb";
			// Backend::serialize_single_file_to_pb(temp_origin_filename, temp_pb_filename);
#endif
...
void Frontend::decrypt_pb_content_stringstream(std::ifstream *ff, std::istringstream *&iss)
{
	// Read the file content into a string
	std::string content((std::istreambuf_iterator<char>(*ff)), std::istreambuf_iterator<char>());

	// Reverse the transformation: XOR each byte with the same value (e.g., 0xAA)
	for (char& c : content)
		c ^= 0xAA;

	// Load the content into the istringstream
	std::istringstream * temp_iss = new std::istringstream(content);
	iss = temp_iss;
}
...
std::string Backend::yosys_exe_path = "";
...
#else
#ifdef HYBRDLINK
			log_cmd_error("Synthesizer is compiled without zlib support, unable to write gzip output.\n");
#else
...
#ifdef HYBRDLINK
			// 仅当传入 -U 参数，并且输出json网表时，才将网表加密
			if(args[0] == "write_json"){
				bin_output = true;
			}
#endif
...
bool Backend::serialize_file_to_pb(std::stringstream &ss, std::vector<std::string> path_seg, std::string &original_filename)
{

	std::string path = yosys_exe_path + "/temp/serialized_files/";
	std::string serialized_file = path + path_seg[path_seg.size()-4] + '_' + path_seg[path_seg.size()-3] + '_' + path_seg[path_seg.size()-2] + '_' + path_seg[path_seg.size()-1];
 	std::ofstream out_file(serialized_file, std::ios::binary);
	if (!out_file.is_open())
	{
		log_error("Unable to serialize .v's content to file '%s'!\n", serialized_file.c_str());
		return false;
	}

    // Simple transformation: XOR each byte with a value (e.g., 0xAA)
    std::string content = ss.str();
    for (char& c : content) {
        c ^= 0xAA;
    }
    // Write the transformed content to the binary file
    out_file.write(content.data(), content.size());

	out_file.close();

	std::ofstream log_f(path + "serialization_log.txt", std::ios::app);
	log_f << original_filename << std::endl;
	log_f.close();

	return true;
}

bool Backend::serialize_single_file_to_pb(std::string &original_filename, std::string &pb_filename){

	std::ifstream *ff = new std::ifstream;
	ff->open(original_filename, std::ifstream::in);
	if (ff->fail()) {
		delete ff;
		ff = nullptr;
	}
	std::stringstream ss;
	ss << ff->rdbuf();
	std::vector<std::string> temp_words = split_str(pb_filename,'/');
	serialize_file_to_pb(ss, temp_words, original_filename);
	return true;
}
