...
#define MAP_JSON "+/hybrdchip/design_elements_H_V.7z"

std::unordered_map<std::string, std::string> Yosys::map_hybrdchip_2_xilinx;
#endif
...
#ifdef HYBRDLINK
// 解析json文件
const std::string get_design_elements_file_path()
{
	std::string file_path = MAP_JSON ;
	rewrite_filename(file_path);
	return file_path;
}

static void parser_hybrdchip_2_xilinx_json()
{
	// only once.
	if (!map_hybrdchip_2_xilinx.empty()) {
		return;
	}
	// read map.json
	Tool::ArchiveTool tool;
	std::vector< Tool::byte_t > buffer;
	tool.extractWithPassword(get_design_elements_file_path(), buffer, KEY);
	std::string buffer_str(buffer.begin(), buffer.end());
	// parser
	std::string err = "";
	json11::Json jsonData = json11::Json::parse(buffer_str, err);
	if (!err.empty()) {
		Yosys::log_error("Parsing json error - %s.\n", err.c_str());
	}
	if (jsonData.is_object()) {
		const std::map<std::string, json11::Json> &file_content = jsonData.object_items();
		for (auto it : file_content) {
			map_hybrdchip_2_xilinx.insert({"\\" + it.first, "\\" + it.second.string_value()});
		}
	} else {
		Yosys::log_error("Parsing json error, the file content is not of type object .\n");
	}
}
#endif
...
int main(int argc, char **argv)
{
	...
	std::string temp_exe_path = argv[0];  // Store yosys excutable's directory for backend usage.
	size_t last_loc = temp_exe_path.find_last_of("/\\");
    if (last_loc != std::string::npos) {
    	temp_exe_path = temp_exe_path.substr(0, last_loc + 1);
    }
	Backend::yosys_exe_path = temp_exe_path;
	if (argc == 2 && (!strcmp(argv[1], "-h") || !strcmp(argv[1], "-help") || !strcmp(argv[1], "--help")))
	{
		...
		// -K 指定父进程id
		printf("    -K\n");
		printf("        Specify the parent process ID (PID).\n");
		printf("\n");
		// -R 标注开启兼容模式
		printf("    -R\n");
		printf("        Enable compatible mode.\n");
		printf("\n");
		// -U 输出json网表时加密
		printf("    -U\n");
	...
	int opt;
#ifdef HYBRDLINK
	// R和U后不接参数，因此写在前半部分。K后需要接进程id，写到后面
	while ((opt = getopt(argc, argv, "MXRUAQTVCSgm:f:Hh:b:o:p:l:L:K:qv:tds:c:W:w:e:r:D:P:E:x:B:")) != -1)
#else
...
#ifdef HYBRDLINK
		case 'K': 
			// 父进程id要在最开始就传入，否则解析verilog时无法建立连接。
			Common::g_father_process_id = optarg;
			break;
		case 'R':
			flag_enable_compatibility_mode = true;
			break;
		case 'U':
			flag_fasm_encryption = true;
			break;
#endif
		case 'M':
			memhasher_on();
	...
	yosys_setup();
#ifdef HYBRDLINK
	// 默认构建hybrdchip to xilinx原语映射关系
	parser_hybrdchip_2_xilinx_json();
#endif
