...
void logv(const char *format, va_list ap)
{
	...
	std::string str = vstringf(format, ap);
	nlohmann::json data = Common::CreateLogJson(LevelCode::ALWAYS_LOG,str,str);
	Common::ConnectAndSendJson(PipeType::LOG, data);
...
void logv_header(RTLIL::Design *design, const char *format, va_list ap)
{
	...
	// 删除header_id最后一个"."，例如2.1. 变为2.1
	if (!header_id.empty() && header_id.back() == '.') {
    	header_id.pop_back();
	}
	header_id = std::string("PHASE") + " " + header_id + " ";
	log2("%s ",false,header_id.c_str());
	Common::CreateLogHeader(header_id);
	logv(format, ap);
	log_flush();
