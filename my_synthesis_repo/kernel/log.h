...
// logv_file_error 带进程通信版
void logv_file_error(const string &filename, int lineno, const char *format, LogData &logdata, va_list ap);

// YS_ATTRIBUTE(format(printf, 1, 2)) 的含义，参数列表中（从1开始），第1个参数为格式化字符串，第2个参数为变参列表
void log(const char *format, ...)  YS_ATTRIBUTE(format(printf, 1, 2));
void log2(const char *format, bool is_send, ...)  YS_ATTRIBUTE(format(printf, 1, 3));
void logv2(const char *format, bool is_send, va_list ap);
void log_header(RTLIL::Design *design, const char *format, ...) YS_ATTRIBUTE(format(printf, 2, 3));

void log_info(const char *format, LogData& logdata,...) YS_ATTRIBUTE(format(printf, 1, 3));
void logv_info(const char *format, LogData& logdata, va_list ap);

void log_warning(const char *format, ...) YS_ATTRIBUTE(format(printf, 1, 2));
void log_warning(const char *format, LogData& logdata,...) YS_ATTRIBUTE(format(printf, 1, 3));
...
void log_error(const char *format, LogData& logdata, ...) YS_ATTRIBUTE(format(printf, 1, 3));
[[noreturn]] void log_file_error(const string &filename, int lineno, const char *format, ...) YS_ATTRIBUTE(format(printf, 3, 4));
void log_file_error(const string &filename, int lineno, LogData &logdata, const char *format, ...) YS_ATTRIBUTE(format(printf, 4, 5));
