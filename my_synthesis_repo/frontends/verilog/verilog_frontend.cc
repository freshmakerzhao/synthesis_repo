...
struct VerilogFrontend : public Frontend {
	...
	void execute(std::istream *&f, std::string filename, std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		// Check if binary file as input
		bool if_bin_input = false;
		if (argidx < args.size())
		{
			std::vector<std::string> temp_words = split_str(args[argidx], '.');
		 	if_bin_input = temp_words[(temp_words.size() - 1)] == "pb";
		}

		extra_args(f, filename, args, argidx, if_bin_input);

#ifdef HYBRDLINK
		// 除用户design外，其他的文件都是原语库（pb），反之亦然，pb文件都是原语库文件，因此这里可通过if_bin_input判断
		// 这里 flag_is_pb_file 是全局变量，在yosys.h中定义，如果想通过参数传递到 judge_design_element_is_hybrdchip 函数中，影响所有调用verilog parser的环节，不建议修改
		flag_is_pb_file = if_bin_input; 
		log_header(design, "Executing Verilog-2005 parser: %s\n", filename.c_str());
		if(log_verbose_level > 9){
		log("Parsing %s%s input from `%s' to synth-mid representation 1.\n",
...
// 相较于原版，增加了 LogData& logdata 参数，传入 log_file_error
void frontend_verilog_yyerror(char const *fmt, LogData& logdata, ...)
{
	va_list ap;
	char buffer[1024];
	char *p = buffer;
