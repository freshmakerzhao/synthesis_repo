...
static RTLIL::Module *process_module(RTLIL::Design *design, AstNode *ast, bool defer, AstNode *original_ast = NULL, bool quiet = false)
{
	...
	if (defer)
		if(log_verbose_level > 9)
			log("Storing synth-mid representation 1 for module `%s'.\n", ast->str.c_str());
	else if (!quiet) {
		if(log_verbose_level > 9)
			log("Generating synth-mid representation 2 for module `%s'.\n", ast->str.c_str());
	}
#else
	if (defer)
...
std::string AstModule::derive_common(RTLIL::Design *design, const dict<RTLIL::IdString, RTLIL::Const> &parameters, AstNode **new_ast_out, bool quiet)
{
	...
	if (!quiet)
#ifdef HYBRDLINK
		log_header(design, "Executing synth-mid 1 frontend in derive mode using pre-parsed synth-mid 1 for module `%s'.\n", stripped_name.c_str());
#else
