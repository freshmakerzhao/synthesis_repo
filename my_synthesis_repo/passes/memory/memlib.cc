...
struct Parser {
	...
	Parser(std::string filename, Library &lib, const pool<std::string> &defines, pool<std::string> &defines_unused) : filename(filename), lib(lib), defines(defines), defines_unused(defines_unused) {
		...
		rewrite_filename(filename);  // convert path to absolut path
		...
		bool is_encrypted = (temp_words[temp_words.size() -1] == "pb") ? true : false;

		std::ifstream *ifile = new std::ifstream;
		ifile->open(filename.c_str(), is_encrypted ? std::ifstream::binary : std::ifstream::in);
		if (ifile->fail()) {
			log_error("failed to open %s\n", filename.c_str());
		}
		...
		if (is_encrypted)	
			Frontend::decrypt_pb_content_stringstream(ifile, iss);

		// Pass ifile content to Parser::infile variable
		if (is_encrypted)
			infile = iss;
		else
			infile = ifile; 
		...
		// Free memory for ifile
		delete ifile;
		ifile = nullptr;
		infile = nullptr;
#endif

		// Serialization targeted lib file
