...
struct IopadmapPass : public Pass {
	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		for (auto module : design->selected_modules())
		{
			...
			dict<SigBit, IdString> pulltype_bits;
			...
			if (!toutpad_celltype.empty() || !tinoutpad_celltype.empty())
			{
				...
				for (auto cell : module->cells()){
					if (cell->type == ID($_TBUF_)) {
						...
						tbuf_bits[bit] = cell;
					} else if (cell->type == ID(PULLDOWN)){
						SigBit bit = cell->getPort(ID::O).as_bit();
						pulltype_bits[bit] = ID(PULLDOWN);
						remove_cells.push_back(cell);
					} else if (cell->type == ID(PULLUP)){
						SigBit bit = cell->getPort(ID::O).as_bit();
						pulltype_bits[bit] = ID(PULLUP);
						remove_cells.push_back(cell);
					} else if (cell->type == ID(KEEPER)){
						SigBit bit = cell->getPort(ID::O).as_bit();
						pulltype_bits[bit] = ID(KEEPER);
						remove_cells.push_back(cell);
					} else {
						for (auto port : cell->connections())
				...
				for (auto wire : module->selected_wires()) {
					if (wire->port_input || wire->port_output){
						for (int i = 0; i < GetSize(wire); i++) {
							...
							if(pulltype_bits.count(wire_bit)){
								auto pulltype = pulltype_bits.at(wire_bit);
								if (pulltype == ID(PULLDOWN))
									wire->set_string_attribute(ID(PULLTYPE), "PULLDOWN");
								else if (pulltype == ID(PULLUP))
									wire->set_string_attribute(ID(PULLTYPE), "PULLUP");
								else if (pulltype == ID(KEEPER))
									wire->set_string_attribute(ID(PULLTYPE), "KEEPER");
							}
						}
					}
				}
				...
				for (auto wire : module->selected_wires())
				{
					...
					for (int i = 0; i < GetSize(wire); i++)
					{
						...
						if (wire->port_input)
						{
							...
							if (tbuf_cell) {
								module->remove(tbuf_cell);
								// module->connect(wire_bit, data_sig);// 会导致多个driver
							}
							if (!toutpad_portname_pad.empty())
