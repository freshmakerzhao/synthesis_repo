...
#ifdef HYBRDLINK
			text = text.substr(0, pos) + "<synthesizer-exe-dirr>/" + text.substr(pos + GetSize(selfdir_name));
#else
		...
		const std::vector<RTLIL::Cell*> &cells, bool show_tempdir, bool sop_mode, bool abc_dress, std::vector<std::string> &dont_use_cells)
{
...
#ifdef HYBRDLINK
	tempdir_name += proc_program_prefix() + "synth-optimizer-XXXXXX";  // Changed folder name for encryption
#else
	...
	{
#ifdef HYBRDLINK
		log_header(design, "Executing synth-optimizer.\n");
#else
...
#ifdef HYBRDLINK
		if(log_verbose_level > 9)
			log("synth-optimizer RESULTS:   %15s cells: %8d\n", it.first.c_str(), it.second);
#else
...
#ifdef HYBRDLINK
		if(log_verbose_level > 9){
			log("synth-optimizer RESULTS:        internal signals: %8d\n", int(signal_list.size()) - in_wires - out_wires);
			log("synth-optimizer RESULTS:           input signals: %8d\n", in_wires);
			log("synth-optimizer RESULTS:          output signals: %8d\n", out_wires);
		}
#else
...
#ifdef HYBRDLINK
		log_header(design, "Executing synth-optimizer pass (technology mapping using ABC).\n");
#else
