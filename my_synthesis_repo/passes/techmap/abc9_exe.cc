...
std::string replace_tempdir(std::string text, std::string tempdir_name, bool show_tempdir)
{
	...
	while (1) {
		...
		if (pos == std::string::npos)
			break;
#ifdef HYBRDLINK
		text = text.substr(0, pos) + "<synth-optimizer-temp-dir>" + text.substr(pos + GetSize(tempdir_name));
#else
	...
	if (selfdir_name != "/") {
		while (1) {
			...
			if (pos == std::string::npos)
				break;
#ifdef HYBRDLINK
		text = text.substr(0, pos) + "<synthesizer-exe-dir>/" + text.substr(pos + GetSize(selfdir_name));
#else
...
struct abc9_output_filter
{
	...
	void next_line(const std::string &line)
	{
		...
		for (char ch : line)  // read input.lut char by char
			next_char(ch);
	}
};
...
)
{
...
#ifdef HYBRDLINK
	FILE *f = fopen(stringf("%s/synth-optimizer.script", tempdir_name.c_str()).c_str(), "wt");  // write to abc.script file
#else
	FILE *f = fopen(stringf("%s/abc.script", tempdir_name.c_str()).c_str(), "wt");  // write to abc.script file
#endif
...
#ifdef HYBRDLINK
	log_header(design, "Executing synth-optimizer2.\n");
#else
...
#ifdef HYBRDLINK
	buffer = stringf("\"%s\" -s -f %s/synth-optimizer.script 2>&1", exe_file.c_str(), tempdir_name.c_str());
	// log("Running synth-optimizer command: %s\n", replace_tempdir(buffer, tempdir_name, show_tempdir).c_str());
#else
	...
	int ret = run_command(buffer, std::bind(&abc9_output_filter::next_line, filt, std::placeholders::_1));	// write to temp file other than .box and .lut
#else
...
#ifdef HYBRDLINK
		log_error("synth-optimizer: cannot open a temporary file for output redirection");
#else
...
#define fd_renumber(from, to) dup2(from, to)
	...
	char *abc9_argv[5];
#ifdef HYBRDLINK
	string tmp_script_name = stringf("%s/synth-optimizer.script", tempdir_name.c_str());
#else
	...
	if (ret != 0) {
		...
		if (check_file_exists(stringf("%s/output.aig", tempdir_name.c_str())))
			log_warning("synth-optimizer: execution of command \"%s\" failed: return code %d.\n", buffer.c_str(), ret);
		else
			log_error("synth-optimizer: execution of command \"%s\" failed: return code %d.\n", buffer.c_str(), ret);
#else
		if (check_file_exists(stringf("%s/output.aig", tempdir_name.c_str())))
...
//		log("\n");
	...
	{
#ifdef HYBRDLINK
		log_header(design, "Executing synth-optimizer2_EXE pass (technology mapping using synth-optimizer2).\n");
#else
		...
		if (!lut_arg.empty()) {
			...
			if (arg.find_first_not_of("0123456789:,") == std::string::npos) {
				...
				lut_file = arg;  // temp file, to be modified
		...
		abc9_module(design, script_file, exe_file, lut_costs, dff_mode,
				...
				box_file, lut_file, wire_delay, tempdir_name);  // create abc9 module
	}
} Abc9ExePass;
