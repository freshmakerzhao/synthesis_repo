...
// SERIALIZE
void write_RTLIL_to_pb(RTLIL::Module *module, const std::string &dst, const std::string lib_name) {
	std::ofstream ofs(dst);
	log_assert(ofs.is_open());
    // Simple transformation: XOR each byte with a value (e.g., 0xAA)
    std::string content = module->design->scratchpad_get_string(lib_name);
    for (char& c : content) {
        c ^= 0xAA;
    }
...
struct Abc9OpsPass : public Pass {
	...
	{
#ifdef HYBRDLINK
		log_header(design, "Executing synth-optimizer2_OPS pass (helper functions for synth-optimizer2).\n");
#else
		...
		for (auto mod : design->selected_modules()) {
			...
			if (!write_lut_dst.empty())
				...
				// write_RTLIL_to_pb(mod, write_lut_dst, "abc9_ops.lut_library");	// To encrypt input.lut
			if (!write_box_dst.empty())
				write_box(mod, write_box_dst);
				// write_RTLIL_to_pb(mod, write_box_dst, "abc9_ops.box_library");  // To encrypt input.box
			if (break_scc_mode)
				break_scc(mod);
			if (prep_xaiger_mode)
