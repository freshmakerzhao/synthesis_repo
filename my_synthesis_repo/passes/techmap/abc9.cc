...
#ifdef HYBRDLINK
		log_header(design, "Executing synth-optimizer2 pass.\n");
#else
...
#else
			run("techmap -wb -map %$abc9 -map +/techmap.pb A:abc9_flop");	// decrypt
#endif // HYBRDLINK
...
#ifndef HYBRDLINK
				run(stringf("techmap -wb -max_iter 1 -map %%$abc9_map -map +/abc9_map.v %s a:abc9_scc_id %%n", dff_mode ? "-D DFF" : ""));	// decrypt
#else
				run(stringf("techmap -wb -max_iter 1 -map %%$abc9_map -map +/abc9_map.pb %s a:abc9_scc_id %%n", dff_mode ? "-D DFF" : ""));	// decrypt
#endif // HYBRDLINK
...
#else
			run("read_verilog -icells -lib -specify +/abc9_model.pb");	// decrypt
#endif // HYBRDLINK
...
#else
				run("techmap -wb -map %$abc9 -map +/techmap.pb");	// decrypt
#endif // HYBRDLINK
		...
		if (check_label("exe")) {
			...
			if (help_mode) {
				...
				for (auto mod : selected_modules) {
					...
					run_nocheck(stringf("write_xaiger -map %s/input.sym %s %s/input.xaig", tempdir_name.c_str(), dff_mode ? "-dff" : "", tempdir_name.c_str()));  // write to .sym and .xaig fiels
...
#else
			run("techmap -wb -map %$abc9_unmap -map +/abc9_unmap.pb"); 	// techmap user design from submod back to original cell	// decrypt
#endif // HYBRDLINK
