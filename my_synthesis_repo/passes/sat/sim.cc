...
struct SimWorker : SimShared
{
	...
	YwHierarchy prepare_yw_hierarchy(const ReadWitness &yw)
	{
		...
		witness_hierarchy(top->module, top, [&](IdPath const &path, WitnessHierarchyItem item, SimInstance *instance) {
			...
			if (item.wire != nullptr) {
				if (paths.count(path)) {
					...
					{
#ifdef HYBRDLINK
						log_warning("Synthesizer witness path `%s` is ambiguous in this design\n", path.str().c_str());
#else
		...
		for (auto &path : paths)
			...
			{
#ifdef HYBRDLINK
				log_warning("Synthesizer witness path `%s` was not found in this design, ignoring\n", path.str().c_str());
#else
			...
			for (auto &clock_input : it->second) {
				...
				if (offset >= signal.offset && (offset - signal.offset) < signal.width) {
					...
					for (int t = 0; t < GetSize(yw.steps); t++) {
						...
						{
#ifdef HYBRDLINK
							log_warning("Synthesizer witness trace has an unexpected value for the clock input `%s` in step %d.\n", signal.path.str().c_str(), t);
#else
	...
	void run_cosim_yw_witness(Module *topmod, int append)
	{
		...
		if (yw.steps.empty()) {
#ifdef HYBRDLINK
			log_warning("Synthesizer witness file `%s` contains no time steps\n", yw.filename.c_str());
#else
	...
	void write_summary()
	{
		...
		json.begin_object();
#ifdef HYBRDLINK
		json.entry("version", "Synthesizer sim summary");
#else
